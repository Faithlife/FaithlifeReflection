...
Consult the [source code](./src/Faithlife.Reflection) for specific details.

## DTOs

A DTO (data transfer object) typically has one or more read/write properties and a default constructor.

The [DtoInfo](./src/Faithlife.Reflection/DtoInfo.cs) static class makes it easy to:
...
To access the information for a DTO, call [DtoInfo.GetInfo](./src/Faithlife.Reflection/DtoInfo.cs). The generic overload, `DtoInfo.GetInfo<T>()`, is slightly more efficient and returns information with stronger types than the non-generic overload, `DtoInfo.GetInfo(Type)`.
...
The [TupleInfo](./src/Faithlife.Reflection/TupleInfo.cs) static class makes it easy to:
...
To access the information for a tuple, call [TupleInfo.GetInfo](./src/Faithlife.Reflection/TupleInfo.cs). The generic overload, `TupleInfo.GetInfo<T>()`, is slightly more efficient and returns information with stronger types than the non-generic overload, `TupleInfo.GetInfo(Type)`.
